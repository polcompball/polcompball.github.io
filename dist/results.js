...
function setBarValue(name, value, right) {
    ...
    if (value > 98) {
        ...
        const separator = outer.parentElement?.querySelector(".divider");
        if (separator)
            separator.style.display = "none";
    }
}
function orderScores(score, users) {
    const ordered = [];
...
function addClosestMatches(users) {
...
async function drawScores(canvas, values, scores) {
...
async function main() {
    ...
    const scores = params.get("score"), digest = params.get("digest") ?? "missing", edition = params.get("edition") ?? "missing";
    if (!scores) {
